# -*- coding: utf-8 -*-
# Updated on Dec 6 02:05:47 2018
#
# @author : selva
#
#
...
#   -----------------
#
#   this script is being developed to extract keywords from a sentence or paragraph that is obtained
#   by the usage of Optical Charecter Recognotion on a piece of scanned document or a picture.
#
#   NLTK is a Natural Language Processing toolkit available in python.
#   We are going to use nltk to extract keywords from a
#   document, analyse the keywords and make necessary
#   adjustments to create a script that
#
# Macro extensions required : sent_tokenize, word_tokenize
...
#specifying the list of words to be used for extraction of data
...
openfileloc =r"E:\OneDrive\Projects\Microsoft Azure\Vishal OCR\output.txt"
outputfileloc =r"E:\OneDrive\Projects\Microsoft Azure\Vishal OCR\Vishal OCR"
file = open(openfileloc, "r")
bulkdata = file.read()
file.close()
dataset = [("--start-- "+i+" -"*(global_series_tolerance+2)+"end") for i in bulkdata.split("\n\n")]

# uses Part-of-speech tagger from NLTK to extract name, other details such as age, gender can be easily extracted
class data_collect:
    def __init__(self):
        self.values = {}
        self.raw = {}
...
def fetch_details(text, options = 1, write = 0):
    #return values [name, age, gender, bmi, blood pressure]
    #options
    #0 returns details as a data_collect object
    #1 prints details_
    #2 prints raw unfiltered fetched data


    #  write parameter toggles saving of file to csv at specified location
    #     file name is automatically set to patient's name


    sentences = sent_tokenize(text) #actual
    #wordsInSent1 = word_tokenize(text)
    wordsInSent1 = word_tokenize(sentences[0]) #actual
    ...
    details.add('URIC ACID [M (2.4 - 6.0) F(3.4 - 7.0) mg/dL]	: ', uricacid[0])
    ...
    if write==1:
        with open(r'E:\OneDrive\Projects\Microsoft Azure\Vishal OCR\nlp.csv','w') as f:
            w = csv.writer(f)
...
def search_around(key_list,words_list, tolerance, depth = 2):
    ...
    depth = min(5,depth)
    #keylist consists of words considered as keywords
    #words_list is the actual tokenised data
    found = []
    flag, f_index = 1,-1
...
# End of functions                                                                                                                                        The following file contains the required search patterns and necessary functions to extract data from medical documents. NLP is a part in this file and is used only to identify the names, medications, symptopms. The data extraction will be accurate only if the document is in the right format,
# These are the neccesary functions that will be                                                                                                          and the name, medications, symptoms and diseases are mentions properly, ie: mentioned starting with a capital letter The format of the document contents can be changed, and it must also be changed in the keywords list in this file If any changes are made thatoverlaps with other keywords, the extraction process will become drastically irrelevant
# required for extracting data from medical documents using appropriate keywords
